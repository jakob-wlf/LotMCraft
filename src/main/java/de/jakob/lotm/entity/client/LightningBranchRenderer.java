...
public class LightningBranchRenderer extends EntityRenderer<LightningBranchEntity> {
    ...
    @Override
    public void render(LightningBranchEntity entity, float entityYaw, float partialTick,
                       PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {

        List<LightningBranchEntity.LightningBranch> branches = entity.getBranches();
        ...
        // Render each branch with appropriate styling based on depth
        for (LightningBranchEntity.LightningBranch branch : branches) {
            List<Vec3> points = branch.getPoints();

            if (points.size() < 2) {
                continue;
            }

            // Color and alpha based on branch depth
            int color = getBranchColor(branch.depth);
            float alpha = getBranchAlpha(branch.depth);
            float width = branch.getWidth();

            renderLightningBolt(consumer, poseStack, points, width, alpha, color);
        }

        poseStack.popPose();
    }

    private int getBranchColor(int depth) {
        return 0x3b19e6;
    }

    private float getBranchAlpha(int depth) {
        // Main branch is fully opaque, sub-branches are more transparent
        return Math.max(0.4f, 1.0f - (depth * 0.2f));
    }
    ...
    private void renderLightningBolt(VertexConsumer consumer, PoseStack poseStack,
        ...
        for (int i = 0; i < points.size() - 1; i++) {
            ...
            float flicker = 1f + 0.4f * (float)Math.sin(System.currentTimeMillis() * 0.01 + i);
    ...
    @Override
    public ResourceLocation getTextureLocation(LightningBranchEntity entity) {
        // Lightning render type doesn't use textures
        return null;
    }

    @Override
    public boolean shouldRender(LightningBranchEntity livingEntity, net.minecraft.client.renderer.culling.Frustum camera, double camX, double camY, double camZ) {
