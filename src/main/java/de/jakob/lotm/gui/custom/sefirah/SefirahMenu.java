...
public class SefirahMenu extends AbstractContainerMenu {

    public final SefirahBlockEntity blockEntity;
    private final Level level;

    public SefirahMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv,
                extraData != null ? inv.player.level().getBlockEntity(extraData.readBlockPos()) : null);
    }

    public SefirahMenu(int pContainerId, Inventory inv, BlockEntity entity) {
        super(ModMenuTypes.SEFIRAH_MENU.get(), pContainerId);

        if (entity instanceof SefirahBlockEntity sefirahBlockEntity) {
            this.blockEntity = sefirahBlockEntity;
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.SEFIRAH_BLOCK.get()); // adjust to your actual block field name
    }
