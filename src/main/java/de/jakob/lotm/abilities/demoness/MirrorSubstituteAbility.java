...
public class MirrorSubstituteAbility extends AbilityItem {

    private static final HashMap<UUID, Integer> figurineNumbers = new HashMap<>();

    public MirrorSubstituteAbility(Properties properties) {
        super(properties, 5f);
    }
    ...
    public static void takeDamage(LivingDamageEvent.Pre event) {
        ...
        if(entity instanceof Player player) {
            int index = player.getInventory().findSlotMatchingItem(new ItemStack(ModItems.MIRROR.get()));
            if(index != -1)
