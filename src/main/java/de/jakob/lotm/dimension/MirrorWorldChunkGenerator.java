...
public class MirrorWorldChunkGenerator extends ChunkGenerator {
    ...
    public static final MapCodec<MirrorWorldChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(
    ...
    public MirrorWorldChunkGenerator(BiomeSource biomeSource) {
        super(biomeSource);
    ...
    private ChunkAccess generateTerrain(ChunkAccess chunk) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                BlockState surfaceBlock = Blocks.GLASS.defaultBlockState();
                ...
                for (int y = minY; y < maxY; y++) {
                    ...
                    } else if (y < terrainHeight || y == terrainHeight) {
                        state = surfaceBlock;
                    } else {
    ...
    public NoiseColumn getBaseColumn(int x, int z, LevelHeightAccessor level, RandomState randomState) {
        ...
        for (int y = minY; y < maxY; y++) {
            if (y < height - 3) {
                ...
                states[y - minY] = Blocks.GLASS.defaultBlockState();
            } else if (y == height) {
                states[y - minY] = Blocks.GLASS.defaultBlockState();
            } else {
