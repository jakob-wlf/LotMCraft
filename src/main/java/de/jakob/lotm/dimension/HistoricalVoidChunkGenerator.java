...
public class HistoricalVoidChunkGenerator extends ChunkGenerator {

    public static final MapCodec<HistoricalVoidChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(ChunkGenerator::getBiomeSource)
            ).apply(instance, HistoricalVoidChunkGenerator::new)
    );

    private final PerlinSimplexNoise surfaceNoise;
    private final PerlinSimplexNoise patchNoise;

    public HistoricalVoidChunkGenerator(BiomeSource biomeSource) {
        super(biomeSource);
    ...
    private ChunkAccess generateTerrain(ChunkAccess chunk) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                double noise = this.surfaceNoise.getValue(worldX * 0.02, worldZ * 0.02, false) * 5; // Much smaller scale and amplitude
                ...
                BlockState surfaceBlock = patchValue > 0 ? Blocks.GRAY_STAINED_GLASS.defaultBlockState() : Blocks.QUARTZ_BLOCK.defaultBlockState();
    ...
    public NoiseColumn getBaseColumn(int x, int z, LevelHeightAccessor level, RandomState randomState) {
        ...
        for (int y = minY; y < maxY; y++) {
            if (y < height - 3) {
                states[y - minY] = Blocks.QUARTZ_BLOCK.defaultBlockState();
            } else if (y < height) {
                states[y - minY] = Blocks.QUARTZ_BLOCK.defaultBlockState();
            } else if (y == height) {
                states[y - minY] = Blocks.GRAY_STAINED_GLASS.defaultBlockState();
            } else {
    ...
    public void addDebugScreenInfo(List<String> list, RandomState randomState, BlockPos blockPos) {
        list.add("Historical Void Generator");
