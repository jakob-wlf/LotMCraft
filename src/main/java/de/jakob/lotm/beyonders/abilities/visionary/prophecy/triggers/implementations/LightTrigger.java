...
public class LightTrigger extends TriggerBase {
    public LightTrigger(ActionBase action, TriggerContextBase context) {
        super(action, context);
    }

    @Override
    public TriggerEnum getType() {
        return TriggerEnum.LIGHT;
    }
    ...
    public int checkTrigger(Level level, LivingEntity entity, UUID casterId) {
        ...
        float light = serverLevel.getRawBrightness(entity.blockPosition(), 0);

        if(numbers.checkOperation(value, light)){
    ...
    public static LightTrigger load(CompoundTag tag,
                                     ActionsEnum actionType,
                                     TriggerContextEnum contextType,
                                     HolderLookup.Provider provider){
        return new LightTrigger(ActionBase.load(actionType, tag, provider),
