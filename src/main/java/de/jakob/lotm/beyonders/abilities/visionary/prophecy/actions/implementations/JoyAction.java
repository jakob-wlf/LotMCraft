...
public class JoyAction extends ActionBase {
    public JoyAction(ActionContextBase context) {
        super(context);
    }

    @Override
    public ActionsEnum getType() {
        return ActionsEnum.JOY;
    }
    ...
    public void action(Level level, LivingEntity entity, UUID casterId) {
        entity.addEffect(new MobEffectInstance(ModEffects.JOY, 20 * 60 * 15));
    }

    public static JoyAction load(CompoundTag tag, HolderLookup.Provider provider) {
        return new JoyAction(ActionContextBase.load(ActionContextEnum.EMPTY, tag, provider));
    }
}
